...
from core.market import FixedPointDollars
import math
import time
import numpy as np
...
class Model:
    '''
    An implementation of the Avellaneda-Stoikov model for
    an inventory-conscious approach to market making
    '''
    ### Model parameters
    ...
    def generate_quotes(self, orderbook_snapshot: OrderBookSnapshot, inventory: int,  volatility: float):
        '''
        Returns the bid quote and ask quotes centered around the reserve price according to A-S.
        '''
    ...
    def normalize_time(self, timestamp):
        '''
        Normalizes time to [0, 1] range where
        0=t_start and 1=t_terminal.
        '''
    ...
    def calc_ask_quote(self, reservation_price: FixedPointDollars) -> FixedPointDollars:
        '''
        Returns the ask quote offset from the reservation price and bound to [0, 1].
        '''
    ...
    def calc_bid_quote(self, reservation_price: FixedPointDollars) -> FixedPointDollars:
        '''
        Returns the bid quote offset from the reservation price and bound to [0, 1].
        '''
        return FixedPointDollars((reservation_price - self.calc_bid_distance())).clamped()
